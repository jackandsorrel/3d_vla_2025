...
class DiffuserActor(nn.Module):
    ...
    def __init__(self,
                 ...
                 group_config=None,
                 filter_points=False,
                 fusion_method=None,  # Fusion method: either concat (on the feature scale) or crossattn
                 pcd_method=None, # pcd method: either clustering or used as patches
                 use_color=False,
                 use_repsurf=False,
                 return_dist=False,
                 mask=False,
                 downsample_points=True,
                 rotation_encoding=False,
        ...
        self.encoder = Encoder(
            ...
            pcd_method=pcd_method,
            group_config=group_config,
            filter_points=filter_points,
            use_color=use_color,
            use_repsurf=use_repsurf,
            return_dist=return_dist,
            downsample_points=downsample_points,
            pcd_backbone=pcd_backbone
        )
        ...
        self.context_linear = nn.Linear(2 * embedding_dim, embedding_dim)
        self.use_color = use_color
        self.mask = mask
        self.rotation_encoding = rotation_encoding
        if self.rotation_encoding:
            self.encoding_linear = nn.Linear(6, 3)

    def encode_inputs(self, visible_rgb, visible_pcd, instruction,
                      curr_gripper, step_id=None, inference = False):
        # Compute visual features/positional embeddings at different scales
        rgb_feats_pyramid, pcd_pyramid = self.encoder.encode_images(
            visible_rgb, visible_pcd, inference = inference
        )
        ...
        if self.rotation_encoding:
            # adding rotation to the positional encoding
            downsampled_pcd = self.encoder.downsample(visible_pcd, 10000)
            rotation = self.encoder.get_rotation(downsampled_pcd, context) # (bs, N, 3)
            context = torch.concat([context, rotation], dim = -1)
            context = self.encoding_linear(context)

        if "rgbonly" in self.fusion_method:
            pass

        else:
            pcd_feats, valid_mask = self.encoder.encode_pcds(visible_pcd, context_feats, context)

            if "concat" in self.fusion_method:
                device = pcd_feats.device
                bs, patch_num, dim = pcd_feats.shape

                if self.mask and not inference:
                    # random masking context_feats
                    max_step = 400000
                    initial_mask_ratio = 0.3
                    current_step = min(step_id, max_step)
                    mask_ratio = initial_mask_ratio * (1.0 - current_step / max_step)
                    
                    num_mask = int(round(patch_num * mask_ratio))

                    random_mask = torch.zeros(bs, patch_num, dtype=torch.bool, device=device)

                    for i in range(bs):
                        indices = torch.randperm(patch_num, device=device) 
                        random_mask[i, indices[:num_mask]] = True
                    
                    random_mask = random_mask.unsqueeze(-1).expand_as(context_feats)
                    context_feats = context_feats.masked_fill(random_mask, 0.0)

                # If dimensions match, prepare concatenated features
                concatenated = torch.cat([context_feats, pcd_feats], dim=-1)  # (B, P, 2*D)
                
                if valid_mask is not None:
                    # Create output tensor with original features as default
                    output_feats = torch.zeros(
                        (bs, patch_num, context_feats.shape[-1] + pcd_feats.shape[-1]),
                        device=device
                    )

                    # For valid patches, use concatenated features
                ...
                context_feats = self.context_linear(output_feats)
    ...
    def compute_trajectory(
        ...
        fixed_inputs = self.encode_inputs(
            rgb_obs, pcd_obs, instruction, curr_gripper, step_id = None, inference = True
        )
    ...
    def forward(
        ...
        """
        if self._relative: # transfer into relative coordinates
            pcd_obs, curr_gripper = self.convert2rel(pcd_obs, curr_gripper)
        if gt_trajectory is not None:
            gt_openess = gt_trajectory[..., 7:] # remaining dimensions
            gt_trajectory = gt_trajectory[..., :7]  # 3d position + quaternion rotation
        ...
        # Convert rotation parametrization(into 6D?)
        ...
        fixed_inputs = self.encode_inputs(
            rgb_obs, pcd_obs, instruction, curr_gripper, step_id
        )
...
class DiffusionHead(nn.Module):
    ...
    def forward(self, trajectory, timestep,
        ...
        """
        Arguments:
            trajectory: (B, trajectory_length, 3+6+X): positions(3D) + rotation(6D) + other features
            timestep: (B, 1)
            context_feats: (B, N, F): N is the number of context points and F is the dimensionality of the features
            context: (B, N, F, 2)
    ...
    def prediction_head(self,
        ...
        Args:
            ...
            sampled_rel_context_pos: A tensor of shape (B, K, F, 2): positional embeddings of the sampled context features
            instr_feats: (B, max_instruction_length, F)
        """
        # Diffusion timestep
