...
class Arguments(tap.Tap):
    ...
    eval_only: int = 0
    trained_episodes: Optional[int] = 0
    ...
    use_pcd: int = 0
    use_pcd2: int = 0
    group_config: str = "1000,1;256,64" 
    filter_points: int = 0
    fusion_method: str = "concat"
    pcd_method: str = "cluster"
    use_color: int = 0
    use_repsurf: int = 0
    return_dist: int = 0
    mask: int = 0
    downsample_points: int = 1
    rotation_encoding: int = 0
    pcd_backbone: str = "pointnet"
...
class TrainTester(BaseTrainTester):
    ...
    def get_datasets(self):
        ...
        print(f"The length of dataset: {len(train_dataset)}")
    ...
    def get_model(self):
        ...
        _model = DiffuserActor(
            ...
            backbone_dir=self.args.backbone_dir,
            ...
            group_config=self.args.group_config,
            filter_points=bool(self.args.filter_points),
            fusion_method=self.args.fusion_method,
            pcd_method=self.args.pcd_method,
            use_color=bool(self.args.use_color),
            use_repsurf=bool(self.args.use_repsurf),
            return_dist=bool(self.args.return_dist),
            mask = bool(self.args.mask),
            downsample_points=bool(self.args.downsample_points),
            rotation_encoding=bool(self.args.rotation_encoding),
            pcd_backbone=self.args.pcd_backbone
        )
    ...
    def train_one_step(self, model, criterion, optimizer, step_id, sample):
        ...
        # Backward pass
        loss = criterion.compute_loss(out) / self.args.accumulate_grad_batches
        loss.backward()
        ...
        # Log
        with torch.no_grad():
            if ((bool(self.args.distributed) and dist.get_rank() == 0) or not bool(self.args.distributed)) and (step_id + 1) % self.args.val_freq == 0:
                self.writer.add_scalar("lr", self.args.lr, step_id)
                self.writer.add_scalar("train-loss/noise_mse", loss.detach().item(), step_id)
    ...
    def evaluate_nsteps(self, model, criterion, loader, step_id, val_iters,
        ...
        for i, sample in enumerate(loader):
            ...
            # if i == 0 and (bool(self.args.distributed) and dist.get_rank() == 0) or not bool(self.args.distributed) and step_id > -1:
        ...
        def print_dict_memory_usage(d, name="values"):
            total_size = 0
            print(f"\n--- Memory breakdown for {name} ---")
            for key, val in d.items():
                if isinstance(val, torch.Tensor):
                    size_bytes = val.numel() * val.element_size()  # bytes
                    total_size += size_bytes
                    print(f"{key}: {size_bytes / 1e6:.2f} MB (shape {val.shape})")
                else:
                    print(f"{key}: Non-tensor (likely scalar)")
            print(f"Total GPU memory used: {total_size / 1e9:.2f} GB\n")

        # Log all statistics
        torch.cuda.empty_cache()
        # print_dict_memory_usage(values)
...
if __name__ == '__main__':
    ...
    args.lr = args.lr * (args.accumulate_grad_batches ** 0.5)
    print(f"lr: {args.lr}")
    print("Arguments:")
    print(args)
    print("-" * 100)
    ...
    pairs = []
    for pair_str in args.group_config.split(';'):
        npoint, nsample = map(int, pair_str.split(','))
        pairs.append((npoint, nsample))
    args.group_config = pairs
    log_dir = args.exp_log_dir
    ...
    # DDP initialization
    if (torch.cuda.is_available()) and bool(args.distributed):
        print("Initializing...")
